...
if (!DATABASE_URL) {
  console.warn(
    "DATABASE_URL not set; skipping Postgres tests. " +
      "Run `docker compose up -d postgres && DATABASE_URL=postgres://queue:queue@localhost:5444/queue deno task test:pg`.",
  );
}
...
pgTest("pg: dispatch and process a simple job", async () => {
...
pgTest("pg: dispatch with delay", async () => {
  await withPg(async (queue) => {
    const done = Promise.withResolvers<void>();
    let receivedAt = 0;
...
pgTest("pg: job context is populated correctly", async () => {
  await withPg(async (queue) => {
    const done = Promise.withResolvers<void>();
...
pgTest("pg: failed job is stored after max attempts", async () => {
  await withPg(async (queue) => {
    const done = Promise.withResolvers<void>();
    let attemptCount = 0;
...
pgTest("pg: global middleware runs for all jobs", async () => {
  await withPg(async (queue) => {
    const done = Promise.withResolvers<void>();
    const order: string[] = [];
    const job: JobHandler<null> = {
...
pgTest("pg: job chain executes in order", async () => {
  await withPg(async (queue) => {
    const results: string[] = [];
    const done = Promise.withResolvers<void>();
    const stepHandler = (
...
pgTest("pg: unique job prevents duplicates", async () => {
  await withPg(async (queue) => {
    let callCount = 0;
    const done = Promise.withResolvers<void>();
    const job: JobHandler<null> = {
...
pgTest("pg: failed job can be retried", async () => {
  await withPg(async (queue) => {
    let attemptCount = 0;
...
pgTest("pg: rate limit middleware throws when exceeded", async () => {
  await withPg(async (queue) => {
...
pgTest("pg: schema is auto-migrated on first use", async () => {
  if (!DATABASE_URL) return;
...
pgTest("pg: custom tablePrefix creates and uses prefixed tables", async () => {
  if (!DATABASE_URL) return;
  const prefix = "qtest_";
  const { default: pg } = await import("pg");
...
pgTest("pg: withoutOverlapping skips concurrent runs", async () => {
  await withPg(async (queue) => {
